...
createdBy.innerHTML += " Federico Carusso";
...
buildToDos([{ description: "Crear nuevo ToDo", complete: false }]);
...
function addToDo() {
  ...
  if (!toDoInput.value) {
    alert(
      "No puede añadir un elemento vacio, por favor complete la descripcion."
    );
    return;
  }
  let td = new ToDo(toDoInput.value);
  toDoItems.push(td);
  toDoInput.value = "";
  displayToDos();
}
...
//mi propia funcion
document.addEventListener("click", (e) => {
  if (e.target.className === "completeCheckbox") {
    let index = e.target.id;
    if (toDoItems[index].clickCount > 0) {
      delete toDoItems[index];
      displayToDos();
    }
    if (toDoItems[index]) {
      toDoItems[index].clickCount++;
    }
  }
});
